...
const electronAPI = {
  ...
  getAudioRecordings: () => ipcRenderer.invoke("get-audio-recordings"),
  deleteAudioRecording: (id: string) => ipcRenderer.invoke("delete-audio-recording", id),
  clearAudioRecordings: () => ipcRenderer.invoke("clear-audio-recordings"),
  isRecordingAudio: () => ipcRenderer.invoke("is-recording-audio"),
  getCurrentAudioRecording: () => ipcRenderer.invoke("get-current-audio-recording"),
  transcribeAudio: (recordingId: string) => ipcRenderer.invoke("transcribe-audio", recordingId),
  processWithAudioTranscript: (recordingId: string) => ipcRenderer.invoke("process-with-audio-transcript", recordingId),

  // Audio event listeners
  onAudioRecordingStarted: (callback: (data: { id: string }) => void) => {
    const subscription = (_: any, data: { id: string }) => callback(data)
    ipcRenderer.on("audio-recording-started", subscription)
    return () => {
      ipcRenderer.removeListener("audio-recording-started", subscription)
    }
  },
  onAudioRecordingStopped: (callback: (recording: any) => void) => {
    const subscription = (_: any, recording: any) => callback(recording)
